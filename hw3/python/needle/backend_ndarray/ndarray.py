...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        if not self.is_compact():
            raise ValueError("Cannot reshape a non-compact array without copying data.")
        ...
        # Create a new NDArray with the same handle but new shape and strides
        return NDArray.make(new_shape, strides=strides, device=self.device, handle=self._handle)
        ### END YOUR SOLUTION
    ...
    def permute(self, new_axes):
        ...
        ### BEGIN YOUR SOLUTION
        # Validate the permutation axes
        if sorted(new_axes) != list(range(len(self.shape))):
            raise ValueError("Invalid permutation axes")
        ...
        # Create a new NDArray with the permuted shape and strides
        return NDArray.make(
    ...
    def broadcast_to(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        # Validate the new_shape
        assert len(new_shape) >= len(self.shape), "New shape must have at least as many dimensions as the original shape"

        # Pad the original shape with 1s on the left if necessary
        original_shape = (1,) * (len(new_shape) - len(self.shape)) + self.shape

        # Check broadcasting compatibility
        for original_dim, new_dim in zip(original_shape, new_shape):
            if original_dim != 1 and original_dim != new_dim:
                raise ValueError(f"Incompatible shapes for broadcasting: {original_shape} vs {new_shape}")

        # Calculate new strides
        original_strides = (0,) * (len(new_shape) - len(self.strides)) + self.strides
        new_strides = tuple(
            0 if original_shape[i] == 1 else original_strides[i]
            for i in range(len(new_shape))
        )

        # Create a new NDArray with the broadcasted shape and adjusted strides
        return NDArray.make(
            new_shape, strides=new_strides, device=self.device, handle=self._handle, offset=self._offset
        )
        ### END YOUR SOLUTION
    ...
    def __getitem__(self, idxs):
        ...
        new_strides = []
        offset = self._offset
        for idx, sl in zip(self.strides, idxs):
            start, stop, step = sl.start, sl.stop, sl.step
            dim_size = (stop - start + step - 1) // step  # Ceil division
            new_shape.append(dim_size)
            new_strides.append(idx * step)
            offset += idx * start

        # Create a new NDArray view
        return NDArray.make(
        tuple(new_shape), strides=tuple(new_strides), device=self.device, handle=self._handle, offset=offset
    )
        ### END YOUR SOLUTION
