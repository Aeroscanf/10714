...
def softmax_loss(Z, y):
    ...
    l = np.log(np.sum(np.exp(Z), axis = 1))

    y_size = Z[np.arange(batch_size), y]
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(0, num_examples, batch):
      ...
      grad = (1/batch) * X_batch.T @ (Z - y_one_hot)
      theta -= lr*grad
    ### END YOUR CODE
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(0, num_examples, batch):
      ...
      y_one_hot = np.zeros((y_batch.shape[0], num_class))
      y_one_hot[np.arange(y_batch.shape[0]), y_batch] = 1
      ...
      Z = np.maximum(X_batch @ W1, 0)

      #compute G2
      logits = Z @ W2
      exp_logits = np.exp(logits - np.max(logits, axis = 1, keepdims=True))
      norm_exp_logits = exp_logits / np.sum(exp_logits, axis = 1, keepdims=True)
      G2 = norm_exp_logits - y_one_hot
