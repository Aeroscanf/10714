...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        ...
        # Propagate partial adjoint to its inputs
        if node.op and hasattr(node.op, 'gradient'):
            input_grads = node.op.gradient(node.grad, node)
            if isinstance(input_grads, Tensor):
              input_grads = [input_grads]
            for k, input_node in enumerate(node.inputs):
                if input_node not in node_to_output_grads_list:
                    node_to_output_grads_list[input_node] = []
                node_to_output_grads_list[input_node].append(input_grads[k])
    
    # The gradients are now stored in the grad field of each node
    ### END YOUR SOLUTION
